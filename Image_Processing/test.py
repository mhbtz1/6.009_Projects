...
def test_inverted_2():
    img = {"height":4, "width":1, "pixels":[231,186,96,50]}
    assert img['pixels'] == lab.inverted({'height':4,'width':1,'pixels':[24, 69, 159, 205]})['pixels']
...
def test_blurred_black_image():
    inp = {'height':6, 'width':5, 'pixels':[0 for j in range(30)]}
    assert inp == lab.blurred(inp,3) and inp == lab.blurred(inp,2)

def test_blurred_centered_pixel():
    inp = lab.load_image('test_images/centered_pixel.png')
    res = lab.blurred(inp,2)
    res2 = lab.blurred(inp,4)
    assert res['pixels'] == lab.load_image('test_results/centered_blur_02.png')['pixels'] and res2['pixels'] == lab.load_image('test_results/centered_blur_04.png')['pixels']
...
def test_edges_centered_pixel():
    inp = lab.load_image('test_images/centered_pixel.png')
    ed = lab.edges(inp)
    assert ed['pixels'] == lab.load_image('test_results/ring_edge.png')['pixels']
